...
`lattice-algo-compress-stack` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for compress analysis with round-trip fixtures, lossless normalization checks, and documented operating limits.
...
The score is deterministic. The current threshold is 155. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 2.
